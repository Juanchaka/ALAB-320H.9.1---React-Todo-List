...
const reducer = (state, action) => {
  switch (action.type) {
    case "ADD_TODO":
      return [
        { userId: 1, id: Date.now(), title: action.payload, completed: false },
        ...state,
      ];
    case "TOGGLE_TODO":
    ...
    case "EDIT_TODO":
      return state.map((todo, index) =>
        index === action.index ? { ...todo, editing: true } : todo
      );
    case "SAVE_TODO":
      return state.map((todo, index) =>
        index === action.index
          ? { ...todo, title: action.payload, editing: false }
          : todo
      );
    default:
      return state;
...
const TodoList = () => {
  ...
  const handleSaveTodo = (index) => {
    dispatch({ type: "SAVE_TODO", index, payload: editingValue });
    setEditingValue("");
  };
